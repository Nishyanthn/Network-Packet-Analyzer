...
def button_click(text):
    ...
    elif text == "Analyse the Packets" and selected_vulnerability == "Malware Infections":

        box_text.delete(1.0, tk.END)  # Clear existing content in box_text
        detect_unusual_ports("okok.pcap", box_text)
    elif text == "Analyse the Packets" and selected_vulnerability == "PII Leak":

        box_text.delete(1.0, tk.END)  # Clear existing content in box_text
        analyze_pcap_for_pii("okok.pcap", box_text)
    elif text == "Analyse the Packets" and selected_vulnerability == "Intrusions and Breaches":

        box_text.delete(1.0, tk.END)  # Clear existing content in box_text
        detect_port_scanning("okok.pcap", box_text)

    elif text == "Analyse the Packets" and selected_vulnerability == "Plaintext Passwords":
         box_text.delete(1.0, tk.END)  # Clear existing content in box_text
...
animal_names = ["why", "Plaintext Passwords",
...
animal_combobox.pack(padx=15, pady=(15, 0))
